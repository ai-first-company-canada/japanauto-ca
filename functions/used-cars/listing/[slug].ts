  ...
  const schemaLD = [
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: makeRow.name, item: `https://japanauto.ca/used-cars/${makeRow.slug}/` },
        { '@type': 'ListItem', position: 3, name: modelRow.name, item: `https://japanauto.ca/used-cars/${makeRow.slug}/${modelRow.slug}/` },
        { '@type': 'ListItem', position: 4, name: cityName, item: `https://japanauto.ca/used-cars/${makeRow.slug}/${modelRow.slug}/${listing.city}/` },
...
<nav aria-label="Breadcrumb" style="padding:12px 16px 0;font-size:12px;color:var(--color-ink-muted)">
  <a href="/" style="color:inherit">Home</a> · <a href="/used-cars/${esc(makeRow.slug)}/" style="color:inherit">${esc(makeRow.name)}</a> · <a href="/used-cars/${esc(makeRow.slug)}/${esc(modelRow.slug)}/" style="color:inherit">${esc(modelRow.name)}</a> · <a href="/used-cars/${esc(makeRow.slug)}/${esc(modelRow.slug)}/${esc(listing.city)}/" style="color:inherit">${esc(cityName)}</a> · <span style="color:var(--color-ink-strong)">${listing.year}${trimSep ? esc(trimSep) : ''}</span>
</nav>
