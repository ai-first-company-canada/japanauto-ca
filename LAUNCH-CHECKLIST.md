...
> **Dry-run 2026-06-12 (коммит 302846f):** audit:launch ЗЕЛЁНЫЙ; каталог и все
> счётчики сайта на живых D1-данных (catalog-live.json, обновляется в predeploy);
> демо-дилеры/ярды и их JSON-LD удалены; в seo-audit.py добавлена regression-сетка
> на сфабрикованные литералы; PII-retention автоматизирован в expire-sweeper;
> audit:launch добавлен в CI и в predeploy. `[~]` = готов скрипт
> `scripts/cleanup-test-data-2026-06-12.sql` — выполнить вручную (см. файл).

## 1. Данные — заменить демо на реальное

- [x] 🤖 `getCatalogForModelCity()` (src/data/catalog-stubs.ts) подключён к
...
- [x] Demo-дилеры из SSG (`/dealers/` индекс: cypress-imports и др.) заменены
...
- [x] Meta descriptions каталог-страниц не утверждают выдуманные количества
      листингов («N used listings» из генератора).
- [x] Thin/near-duplicate шаблонный контент по city-variant страницам
...
- [x] 🤖 Sanity: `dist/robots.txt` открыт (гейт упадёт, если кто-то заменит
...
- [x] №30 — Organization-логотип в page-shell.ts указывает на несуществующий
      `/logo.png` (есть только logo.svg).
- [x] №31 — Breadcrumb listing-detail ссылается на legacy-URL c 301.
- [x] №19 — CI deploy.yml не гоняет `audit:seo` (локальный `npm run deploy`
      гоняет; CI-путь должен тоже + `audit:launch` после запуска).

## 3b. Data retention (PII-минимизация)

- [x] Настроить периодическую чистку (Cloudflare Cron Trigger / Worker — Pages
...
- [x] `JWT_SECRET` задан в Pages-проекте и **длиной ≥ 32 символов** — короче
...
- [x] `CLOUDFLARE_ACCOUNT_ID` + `CLOUDFLARE_IMAGES_API_TOKEN` заданы.
- [x] `DAILY_IP_HASH_SALT` задан (не дефолт/пустой).
- [ ] Stripe: боевые ключи + webhook на прод-URL (когда платежи включаются).

## 5. Финальная проверка

- [x] 🤖 `npm run audit:launch` — зелёный.
- [x] `npm run deploy` — predeploy-гейты зелёные.
