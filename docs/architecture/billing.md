...
Monetization layer for the marketplace: per-dealer subscription tier/status (free|pro) that gates listing-creation rate limits, plus two paid-placement products — per-listing "boost" (self-serve, Stripe one-time) and per-(make,city) "featured slots" (admin/direct-sales). Subscriptions and Stripe payment processing are schema + skeleton only; nothing is wired end-to-end yet.
